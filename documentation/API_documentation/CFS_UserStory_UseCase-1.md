...
|Begins when|The BA invokes the Call Forwarding Signal API to get the information on the status of the Call Forwarding service for his Customer's phone number.| M |
|Step 1|The OP identifies the BA as a valid CFS API Consumer| M |
|Step 2|The OP verifies the User Consent| M|
|Step 3|The BA invokes the CFS API Producer providing the Customer's phone number| M |
|Step 4|The CFS API Producer checks if the Customer's phone number has an active Call Forwarding Service| M |
|Ends when|The CFS API Producer notifies the BA about the status of the Call Forwarding Service| M |
|Post-conditions|The BA is aware of the status for the Call Forwarding Service for the Customer's phone number | M |
